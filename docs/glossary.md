...
| Fallback extraction | Tree-sitter symbols and imports, without call resolution, for projects a SCIP indexer can't build. Its edges are `import-scoped` or `name-match`. |
...
| Index hash | An identifier for an index. The same commit and configuration must produce the same hash. |
...
| Provenance | Where a fact came from: file, line range, and commit. |
| Renderer | Turns findings and summaries into outputs: documents, llms.txt or AGENTS.md files, and backlog items. |
| Resolution level | How an edge was established: `resolved` by a compiler (SCIP), `import-scoped` (unique within imported files), or `name-match` (a repo-wide guess). |
